...
const EMPTY_FILTERS: DeviceTableFilters = {
  distributorIds: [],
  customerIds: [],
  departmentIds: [],
};
...
export function useDevicesTable(initialPageSize = 5) {
  ...
  const [filters, setFiltersValue] = useState<DeviceTableFilters>(EMPTY_FILTERS);
  ...
  const setFilters = (value: DeviceTableFilters) => {
    setCurrentPage(1);
    setFiltersValue(value);
  };

  const { data, isPending, error } = useQuery<DeviceListResponse, Error>({
    queryKey: ["devices", "list", { page: currentPage, pageSize, query: queryValue, filters }],
    refetchOnMount: true,
    queryFn: async () => {
